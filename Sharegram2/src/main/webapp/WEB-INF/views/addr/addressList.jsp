     ...
     <script>
            	$(function(){
            		...
            		$(document).on("click", "#adDetailTb :checkbox", function(){
            			...
            			updateStarAdd($(this).next().next().val(), addStar, $(this).attr("addrGroupNo"));            		
            		})
            		
            		// Modal < 그룹 수정시 필요한 groupNo, addrGroupName 전달
            				...
            				if(empList.length == 0){
            					...
            					$.each(empList, function(i, empAddr){
            						value += 		'<tr>'
                                        	...
                                        	if(empAddr.empPhone != null){
                                        		 value   += '<td>' + empAddr.phone + '</td>';
                                        	}else{
                                        		 value 	 += '<td>'+ '' +'</td>'; 
                                        	}
                                    value   +=  '</tr>'	;                                        	
            		...
            		$.ajax({
            			...
            			success:function(resData){
                           ...
                           if(resData.phone != null){
                        	   value += 			'<td colspan="4" class="fontMiddleAdd">&nbsp;&nbsp;' + emp.phone + ' </td>';			               			  
                           }
            	...
            	// 내 연락처 목록클릭 시 조회 테이블
            	function selectAddTbList(addrGroupNo, cpage){
            			...
            			success : function(result){
            				...
            				if(list.length == 0){
            					...
            					for(let i=0; i<list.length; i++){
            					 ...
            					 if(pi.currentPage != 1){
                        			pageValue += "<button class='btn btn-sm btn-outline-primary' onclick='selectAddTbList("+ addrGroupNo + ", "  + (pi.currentPage - 1) + ")'>&lt;</button>"	
                        		}
            	...
            	// 내연락처 & 별표연락처 상세보기
            	function selectDetailMyAdd(addressNo, addrGroupNo){
            		$.ajax({
            			url : "detailMyAdd.ad",
            			type:  "post",
            			data:{
            			...
            			success:function(result){
        				    ...
        				    value += '<thead>'
                        		 	+		'<tr>'
                        		 	+	        '<th class="Addstar">'
                        		 	+				'<input type="checkbox" name="addStar" value="Y" id="rate2" addrGroupNo="' + addrGroupNo + '"';
